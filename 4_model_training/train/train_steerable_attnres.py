...
DEFAULT_ATTNRES_MODULES_TO_SAVE = {
    "pre_attn": ["attn_res_proj", "attn_res_norm", "attn_res_gate"],
    "pre_mlp": ["mlp_res_proj", "mlp_res_norm", "mlp_res_gate"],
}
...
def build_attnres_modules_to_save(
    *,
    attnres_enabled: bool,
    apply_pre_attn: bool,
    apply_pre_mlp: bool,
) -> list[str]:
    if not attnres_enabled:
        return []
    modules: list[str] = []
    if apply_pre_attn:
        modules.extend(DEFAULT_ATTNRES_MODULES_TO_SAVE["pre_attn"])
    if apply_pre_mlp:
        modules.extend(DEFAULT_ATTNRES_MODULES_TO_SAVE["pre_mlp"])
    return modules


def apply_attnres_config_overrides(config: Any, args: argparse.Namespace) -> dict[str, Any]:
    text_config = getattr(config, "text_config", None)
    if text_config is None:
        raise RuntimeError("Molmo2 config is missing text_config; cannot enable AttnRes")
    text_config.attnres_enabled = bool(args.attnres_enabled)
    text_config.attnres_layers_per_block = int(args.attnres_layers_per_block)
    text_config.attnres_apply_pre_attn = bool(args.attnres_apply_pre_attn)
    text_config.attnres_apply_pre_mlp = bool(args.attnres_apply_pre_mlp)
    text_config.attnres_gate_init = float(args.attnres_gate_init)
    return {
        "enabled": bool(text_config.attnres_enabled),
        "layers_per_block": int(text_config.attnres_layers_per_block),
        "apply_pre_attn": bool(text_config.attnres_apply_pre_attn),
        "apply_pre_mlp": bool(text_config.attnres_apply_pre_mlp),
        "gate_init": float(text_config.attnres_gate_init),
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Filtered-data Molmo2 LoRA + AttnRes training")
    ...
    parser.add_argument("--skip_save_eval_before_step", type=int, default=0, help="Do not save checkpoints or run eval before this step")
    parser.add_argument("--attnres_enabled", dest="attnres_enabled", action="store_true")
    parser.add_argument("--no_attnres", dest="attnres_enabled", action="store_false")
    parser.add_argument("--attnres_layers_per_block", type=int, default=4)
    parser.add_argument("--attnres_gate_init", type=float, default=0.0)
    parser.add_argument("--attnres_apply_pre_attn", dest="attnres_apply_pre_attn", action="store_true")
    parser.add_argument("--attnres_disable_pre_attn", dest="attnres_apply_pre_attn", action="store_false")
    parser.add_argument("--attnres_apply_pre_mlp", dest="attnres_apply_pre_mlp", action="store_true")
    parser.add_argument("--attnres_disable_pre_mlp", dest="attnres_apply_pre_mlp", action="store_false")
    ...
    parser.set_defaults(
        ...
        attnres_apply_pre_attn=True,
        attnres_apply_pre_mlp=True,
    )
    ...
    if int(args.attnres_layers_per_block) <= 0:
        raise ValueError("--attnres_layers_per_block must be > 0")
    if bool(args.attnres_enabled) and not (bool(args.attnres_apply_pre_attn) or bool(args.attnres_apply_pre_mlp)):
        raise ValueError("AttnRes is enabled but both pre-attn and pre-mlp hooks are disabled")
    return args
...
def main() -> int:
    ...
    print(f"[INFO] loading config model_path={args.model_path}")
    model_config = AutoConfig.from_pretrained(args.model_path, trust_remote_code=True)
    attnres_report = apply_attnres_config_overrides(model_config, args)
    ...
    attnres_modules_to_save = build_attnres_modules_to_save(
        attnres_enabled=bool(args.attnres_enabled),
        apply_pre_attn=bool(args.attnres_apply_pre_attn),
        apply_pre_mlp=bool(args.attnres_apply_pre_mlp),
    )
    lora_cfg = LoraConfig(
        r=int(args.lora_r),
        ...
        modules_to_save=(attnres_modules_to_save or None),
    )
    ...
    setup_report = {
        ...
        "lora": {
            ...
            "modules_to_save": list(lora_cfg.modules_to_save or []),
    ...
    while step < total_micro_steps:
        ...
        for batch in loader:
            ...
            batch_ids = batch.pop("_meta_ids", []) if isinstance(batch, dict) else []
            batch_targets = batch.pop("_meta_targets", []) if isinstance(batch, dict) else []
            ...
            with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=use_amp):
                ...
                if not torch.isfinite(outputs.logits).all() or not torch.isfinite(loss_full):
                    diag = {
                        "step": int(step),
                        "epoch": int(epoch),
                        "epoch_step": int(epoch_step),
                        "batch_sources": [str(item) for item in batch_sources],
                        "batch_ids": [str(item) for item in batch_ids],
                        "batch_targets": [str(item) for item in batch_targets],
                        "valid_label_tokens": int((labels[:, 1:] != -100).sum().detach().cpu().item()),
                        "logits_all_finite": bool(torch.isfinite(outputs.logits).all().detach().cpu().item()),
                    }
                    print(f"[ERROR] non-finite training loss/logits: {json.dumps(diag, ensure_ascii=False)}", flush=True)
                    raise RuntimeError("non-finite training loss/logits")
            ...
            # Respect --skip_save_eval_before_step: no saves or evals before this step
            skip_before = int(args.skip_save_eval_before_step)
            if skip_before > 0 and step < skip_before:
                do_save = False
                do_eval = False
